...
export default function TiposEditalPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/tipos-edital`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = formData.id ? `${API_URL}/tipos-edital/${formData.id}` : `${API_URL}/tipos-edital`;
    ...
    try {
      ...
      alert('Erro ao salvar tipo de edital');
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_URL}/tipos-edital/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) fetchData();
  ...
  return (
    <PermissionGuard requiredPermission="TIPOS_EDITAL_LISTAR">
      <div className="space-y-10 animate-in fade-in duration-700">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/40 backdrop-blur-md p-8 rounded-[40px] border border-white/20 shadow-sm">
          <div>
            <h1 className="text-4xl font-black text-primary-mucuna tracking-tighter italic uppercase">Tipos de Edital</h1>
            <p className="text-primary-mucuna/60 font-bold mt-1 uppercase text-xs tracking-[0.2em]">Categorização de Processos</p>
          </div>
          <button 
          ...
          >
            ...
            <span className="relative z-10">Novo Tipo</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? (
            <div className="col-span-full py-40 text-center bg-white/40 backdrop-blur-md rounded-[48px] border border-white/20 shadow-sm animate-pulse">
              ...
              <p className="text-primary-mucuna/40 font-black uppercase tracking-[0.3em] text-sm">Sincronizando Categorias...</p>
            </div>
          ) : items.length === 0 ? (
            <div className="col-span-full bg-white/40 backdrop-blur-md p-20 rounded-[48px] text-center border-2 border-dashed border-primary-mucuna/10">
              <h3 className="text-2xl font-black text-primary-mucuna italic tracking-tighter uppercase mb-2">Nenhum Tipo Configurado</h3>
              <p className="text-primary-mucuna/40 font-bold uppercase text-[10px] tracking-widest">Inicie a estrutura de editais do sistema</p>
            </div>
          ) : (
            items.map(c => (
              <div key={c.id} className="bg-white/70 backdrop-blur-xl rounded-[48px] p-10 shadow-2xl shadow-primary-mucuna/5 border border-white hover:shadow-primary-mucuna/10 transition-all group relative overflow-hidden flex flex-col justify-between hover:-translate-y-2 duration-500">
                ...
                <div className="relative z-10 space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-primary-mucuna/30 font-black uppercase tracking-[0.2em] mt-2 italic">Modelo de Publicação</p>
                  </div>
        ...
        <Modal 
          ...
          title={`${formData.id ? 'Ajustar' : 'Novo'} Tipo`}
          subtitle="Configuração de Finalidade do Edital"
          maxWidth="max-w-md"
        >
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-primary-mucuna/30 uppercase tracking-[0.2em] ml-2">Identificação do Tipo</label>
              <input 
                type="text" required 
                value={formData.nome}
                ...
                placeholder="Ex: Edital de Inscrição"
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4">
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-r from-accent-mucuna to-support-mucuna opacity-0 group-hover:opacity-10 transition-opacity" />
                <span className="relative z-10">Confirmar Tipo</span>
              </button>
            </div>
          </form>
          </Modal>
