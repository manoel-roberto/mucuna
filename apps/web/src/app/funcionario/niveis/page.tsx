  ...
  return (
    <PermissionGuard requiredPermission="NIVEIS_LISTAR">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Gestão de Níveis</h1>
          <p className="text-slate-500 font-medium">Cadastre e gerencie os níveis dos cargos (Superior, Médio, etc.)</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {niveis.map(n => (
          <div key={n.id} className="bg-white p-8 rounded-[32px] shadow-xl border border-slate-100 hover:border-emerald-200 transition-all group relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-4">
              ...
              <h3 className="text-xl font-black text-slate-900 leading-tight">{n.nome}</h3>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] shadow-2xl max-w-lg w-full p-10 space-y-6">
            <h2 className="text-2xl font-black text-slate-900 italic">{formData.id ? 'Editar' : 'Novo'} Nível</h2>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-sm font-black text-slate-400 uppercase tracking-widest ml-1">Nome do Nível</label>
                <input 
                  type="text" 
                  required 
                  value={formData.nome}
