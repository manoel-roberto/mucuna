...
export default function NiveisPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/niveis`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) setNiveis(await res.json());
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = formData.id ? `${API_URL}/niveis/${formData.id}` : `${API_URL}/niveis`;
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Excluir este nível? Editais vinculados podem ser afetados.')) return;
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_URL}/niveis/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) fetchData();
  ...
  return (
    <PermissionGuard requiredPermission="NIVEIS_LISTAR">
      <div className="space-y-10 animate-in fade-in duration-700">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/40 backdrop-blur-md p-8 rounded-[40px] border border-white/20 shadow-sm">
          <div>
            <h1 className="text-4xl font-black text-primary-mucuna tracking-tighter italic uppercase">Gestão de Níveis</h1>
            <p className="text-primary-mucuna/60 font-bold mt-1 uppercase text-xs tracking-[0.2em]">Escolaridade e Requisitos de Cargo</p>
          </div>
          <button 
          ...
          >
            ...
            <span className="relative z-10">Novo Nível</span>
          </button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {niveis.map(n => (
            <div key={n.id} className="bg-white/70 backdrop-blur-xl rounded-[48px] p-10 shadow-2xl shadow-primary-mucuna/5 border border-white hover:shadow-primary-mucuna/10 transition-all group relative overflow-hidden flex flex-col justify-between hover:-translate-y-2">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <h3 className="text-2xl font-black text-primary-mucuna leading-none group-hover:text-accent-mucuna transition-colors italic tracking-tighter uppercase">{n.nome}</h3>
                
                <div className="pt-4 flex items-center justify-between border-t border-primary-mucuna/5 pt-8 mt-6">
                  <button 
                    onClick={() => { setFormData({ id: n.id, nome: n.nome }); setShowModal(true); }}
      ...
      <Modal 
        ...
        title={`${formData.id ? 'Editar' : 'Novo'} Nível`}
        subtitle="Gerenciamento de Requisitos"
        maxWidth="max-w-lg"
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-1">
            <label className="text-sm font-black text-primary-mucuna/40 uppercase tracking-widest ml-1">Nome do Nível</label>
            <input 
              type="text" 
              required 
              value={formData.nome}
