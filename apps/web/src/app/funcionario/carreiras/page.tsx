    ...
    <PermissionGuard requiredPermission="CARREIRAS_LISTAR">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-medium">Cadastre e gerencie as carreiras funcionais do sistema</p>
        </div>
        <button 
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] shadow-2xl max-w-lg w-full p-10 space-y-6">
            <h2 className="text-2xl font-black text-slate-900 italic">{formData.id ? 'Editar' : 'Nova'} Carreira</h2>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-sm font-black text-slate-400 uppercase tracking-widest ml-1">Nome da Carreira</label>
                <input 
                  type="text" 
                  required 
                  value={formData.nome}
