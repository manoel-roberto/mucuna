...
export default function CarreirasPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/carreiras`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) setCarreiras(await res.json());
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = formData.id ? `${API_URL}/carreiras/${formData.id}` : `${API_URL}/carreiras`;
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Excluir esta carreira? Editais vinculados podem ser afetados.')) return;
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_URL}/carreiras/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) fetchData();
    ...
    <PermissionGuard requiredPermission="CARREIRAS_LISTAR">
      <div className="space-y-10 animate-in fade-in duration-700">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/40 backdrop-blur-md p-8 rounded-[40px] border border-white/20 shadow-sm">
          <div>
            <h1 className="text-4xl font-black text-primary-mucuna tracking-tighter italic uppercase">Gestão de Carreiras</h1>
            <p className="text-primary-mucuna/60 font-bold mt-1 uppercase text-xs tracking-[0.2em]">Administração de Linhas Funcionais</p>
          </div>
          <button 
          ...
          >
            ...
            <span className="relative z-10">Nova Carreira</span>
          </button>
        </div>
      ...
      <Modal 
        ...
        title={`${formData.id ? 'Editar' : 'Nova'} Carreira`}
        subtitle="Gestão de Linha Funcional"
        maxWidth="max-w-lg"
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-1">
            ...
            <input 
              ...
              placeholder="Ex: Carreira de Magistério"
